#!/usr/bin/env python3
"""
Fine-tune T5 for grammar correction from a JSONL file.

Usage (GPU recommended):
  pip install transformers datasets accelerate sentencepiece torch
  python train_gec.py --data data/my_pairs.jsonl --output outputs/gec-finetuned

Each JSONL line: {"source": "wrong sentence", "target": "corrected sentence"}
"""
...
def main() -> None:
    ...
    p.add_argument("--data", type=Path, required=True, help="Path to my_pairs.jsonl")
    p.add_argument("--output", type=Path, default=Path("outputs/gec-finetuned"))
    p.add_argument(
        "--model",
        default="vennify/t5-base-grammar-correction",
        help="Base checkpoint (T5 seq2seq)",
    )
    p.add_argument("--epochs", type=float, default=5.0)
    p.add_argument("--batch-size", type=int, default=8)
    p.add_argument("--lr", type=float, default=3e-5)
    p.add_argument("--no-fp16", action="store_true", help="Disable fp16 if unstable on GPU")
    args = p.parse_args()
    ...
    print("Done. Point Bloom to:", out.resolve())
